    ...
    def _rows(self, theta, R, active, wts, k, c, e):
        ...
        for sgn in (+1.0, -1.0):
            ...
            for d, (s, n) in self._design_run(theta, R, active, p,
                                              k, c, e).items():
                # s is ALREADY the mean over that row's shots for THIS sign
                # branch, and it estimates sum_x (p_x/mass) f_x(row d). Times
                # `mass` it is the unnormalised weighted sum for the branch, and
                # the two branches must then be SUMMED:
                #
                #     E_d = mass+ * s+  -  mass- * s-
                #
                # An earlier version wrote `acc[d] += sgn*mass*s*n` and divided
                # by the POOLED count n+ + n-, which AVERAGES the two branches
                # instead of summing them. With n+ ~ n- that halves E_d - the
                # measured 2x gradient deficit - and because n+/n- varies per
                # row and per parameter block it is not a uniform scale but a
                # DISPERSION (0.42 vs 0.53 across two blocks), which Part X says
                # turns the direction. Worse, the sign balance DRIFTS as the
                # residuals change during training, so the distortion moves with
                # the optimiser: that is why cos looked fine at the start point
                # and the loop still stalled at ~10% of the attainable gap.
                acc[d] += sgn * mass * s
                cnt[d] += n
        return acc, cnt
...
def line_search_step(proto, th, y, act, g, mse_fn, t_max=0.6, depth=10):
    """Backtracking line search. THIS IS WHAT MAKES THE LOOP CONVERGE.

    The accept/reject trust region CARRIED its radius between epochs - 1.15x on
    accept, 0.7x on reject - so one bad patch collapsed it permanently and
    progress per epoch was capped at R -> 0. Measured: R hit 0.000 by epoch 93
    and the loop froze 0.0784 above a floor that BFGS reaches from the same
    point in 25 iterations. The endpoint was never a local minimum, which is
    what said the loop was stopping for its own reasons rather than because the
    basin was exhausted.

    Re-choosing the step length FROM SCRATCH each epoch costs one bad estimate
    one epoch instead of the whole run: 16.5% -> 100.0% of the attainable gap,
    and the endpoint became a genuine local minimum.

    Eight tuning variants failed to find this and are recorded so they are not
    retried: Newton step, plain gradient, Cauchy step, block cycling on and off,
    S ~ R^-4 shot schedule, an adaptive floor R_min = sigma/(kappa ||g|| sqrt S),
    and shots ~ 1/||g||^2. All landed at 9-17%.

    COST: `depth` extra loss evaluations per epoch, each one residuals() circuit
    on hardware. Measured 6.9 on average, so ~9.9 circuits per epoch total, and
    every term is independent of |D| and of M.

    Also required, and separate from this: SENSE at a fixed radius and STEP at
    the searched length. Tying the measurement radius to the step size is a
    category error - small R gives a gradient pointing BACKWARDS (measured
    cos -0.29 at R=0.05), large R gives a good direction but an overshooting
    step, and held at the compromise the loop freezes entirely.
    """
    gi = np.asarray(g)[act]
    ng = float(np.linalg.norm(gi))
    if ng <= 1e-12:
        return np.array(th, float), 0.0
    u = gi / ng
    base = mse_fn(th)
    t = t_max
    for _ in range(depth):
        cand = np.array(th, float)
        cand[act] += -t * u
        if mse_fn(cand) < base:
            return cand, t
        t *= 0.5
