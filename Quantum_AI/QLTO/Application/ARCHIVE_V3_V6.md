# QLTO — archive: the V3 and V6 record (Parts I–II)

**This is the historical record.** It is Parts I and II of what was one file, kept
verbatim because project rule R2 requires withdrawn claims to stay in place beside
whatever refuted them. It is not the current state of the project — see
[`RESEARCH_NOTES.md`](RESEARCH_NOTES.md) for Parts III–VII.

**Part I — V3** is the V3-era record: the one-circuit sensing oracle, the quantum
walk, and the theory worked out around them. It was originally `nisq_v3.py`'s
module docstring.

**Part II — V6, calibration, and the frontier** covers the log-width design
register, `twirl_cal`, and the tier-A re-measurement of claims Part I and the
READMEs had asserted on weaker evidence.

## Verdicts in here that later parts SUPERSEDE

Read these with the correction, not on their own:

| claim in here | superseded by | what changed |
|---|---|---|
| the walk "roughly breaks even" against a classical decode; whether the coherent step pays is open | **Part VI** | not open — the walk as built is `Σᵢ(βXᵢ + αᵢZᵢ)`, a tensor product, zero entanglement, O(n) classical. Every walk-vs-classical number here compared a classical computation with a classical computation |
| v99's tunnelling result read as evidence about the shipped walk | **Part VI** | PART 4 used a Hamming-weight potential (degree ≥ 2); the shipped walk is degree-1. Different constructions |
| barrier width 0.96/0.99 "of the path" as the walk's precondition | **Parts VI, VII** | the binding constraint is the potential DEGREE, not barrier width or the mixer |
| G, or QPE ladder depth, cited as limits on QLTO | **Part V** | those are SUBSTRATE costs paid by every competing method; they bound the application, not the method |

Everything else here stands. Logs live in `supplement/results/`; the script that
produced each is indexed in `supplement/README.md`.
...
# Part II — V6, calibration, and the frontier

Part I is the V3 record. This part covers the later line and, more importantly,
the **re-measurement of claims that were asserted on tier-C evidence** and are now
tier A. Several long-standing statements in the READMEs turned out to be right for
the wrong reasons, and a few were simply wrong. All of it is kept beside what
refuted it.

## The V6 caveats, re-measured on circuits

The caveat quoted beside every V6 cost claim read: *"in the wide-ansatz regime V6's
variance exponent is 1.94 against parameter-shift's 2.00 (`v82`) — the advantage is
circuits, not shots."* The **conclusion holds**. Both stated reasons were wrong and
the evidence was the wrong tier.

**`v82` is tier C.** Zero `QuantumCircuit`, `AerSimulator` or `Statevector`
constructs anywhere in the file — a synthetic Gaussian estimator model whose own
docstring says it *"says nothing about circuit depth or hardware."* It was
constraining a tier-A benchmark.

**It measured two regimes and only the pessimistic one was quoted.**

```
REGIME: per-component fixed   V6 alpha = +1.940    p-shift alpha = +2.000
REGIME: total norm fixed      V6 alpha = +1.006    p-shift alpha = +2.000
```

**`v108` measured which regime the real landscape occupies.** On Heisenberg with
`efficient_su2`: `|grad E| ~ M^0.130`, `mean|g_i| ~ M^-0.491`. That is the
*total-norm-fixed* regime (which predicts 0.0 and -0.5), **not** the one the caveat
cited. So the caveat quoted the wrong exponent.

**And the favourable regime still does not rescue the shot axis**, because `v82`
excludes the finite-radius bias *by construction* and the bias is what dominates:
`v108` measured `bias^2 = 1.82e-1` against `tr(Cov) = 6.17e-2` at M=16. A
variance-only comparison flatters V6 by exactly the term that costs it.

**Measured on circuits** (`v109`), cos against the exact gradient at matched total
shots, R swept and V6 given its best radius at every point:

```
total shots   p-shift cos   V6 best cos
      1024      0.85977       0.73645
     16384      0.98880       0.97270
    262144      0.99934       0.99689
```

Parameter-shift ahead at every budget. V6 needs roughly **4x more total shots** for
the same direction quality, at **32x fewer circuits**. The gap **widens with M**
(`v110`): +0.019 -> +0.025 -> +0.144 at M = 16, 32, 64.

**Two escapes closed.** Annealing the trust region does not rescue it — at matched
total shots with an identical step rule, parameter-shift descent still lands lower
(`v112`; underpowered at 3 seeds and a reps=1 ansatz ceiling, so do not lean on the
magnitude). And the radius rescale exponent is **not** a hidden schedule: swept for
the first time in `v111`, `p=0.5` wins at large M and `p=0` collapses to cos 0.095
at M=64, because `sqrt(M)*R_eff` is what the linearisation sees and `p=0.5` is
exactly the exponent that holds it fixed. *(I predicted the optimal p would fall
with M. It rises. The prediction is left in `_radius`'s docstring beside the
sweep.)*

**Restated honestly**, this is a currency claim rather than a concession: V6
reaches the same gradient quality on ~4x more shots and 16-64x fewer circuits. On
per-task or `rep_delay`-dominated billing that is a large net saving (`v21`); on
pure per-shot billing it is not. Parameter-shift additionally has a hard floor of
`2MG` circuits per gradient — one shot each, minimum — which V6 does not.

## The calibration line: four withdrawals and the frontier comparison

`twirl_cal` learns device Hamiltonian coefficients by putting Pauli twirls in
superposition. A conjugation flips coefficient signs, so **a twirl is a design
row** and the device supplies the evolution exactly in every branch — no product
formula, no Trotter bias. Four things claimed about it are withdrawn.

**1. Not `O(1)` as originally stated.** `v104` measured the shipped path at exactly
**8N** circuits (24, 32, 40, 48, 56, 64 at N=3..8). The growth was entirely the
default observable set — N single-qubit Z plus N single-qubit X, one circuit each.
Fixed in `v105`; see below.

**2. The 3.0% headline was a single unseeded draw.** `v102`: the seed mean at that
configuration is **6.7% +- 1.0%**, and 1.9% +- 0.15% at 8x the shots.

**3. "8x shots, no gain — bias-limited" is refuted at the operating point.** Error
falls **3.46x** for 8x shots at T=0.25, so it is *shot*-limited there. The
diagnosis does hold at T >= 0.5 where truncation dominates, which is why it looked
right.

**4. "10x the accuracy at 6.7x fewer circuits" against `qlto_hl.fit()` is withdrawn
with no replacement.** It compared a lucky-draw mean against a worst case, across a
9.6x shot gap, and cold-start against a +-0.4 warm start. No ratio is offered
because there is no working baseline: `fit()` with library defaults **does not
converge** on this model at any T from 0.25 to 2.0 (`P(truth)` = 1.0000 throughout,
so the landscape is fine), and `v100`'s converging configuration solves a different
problem — a *local refiner* going 0.0597 -> 0.0172 (`v104`).

**Also established**: `device_reps` is a simulation artefact, not a protocol
parameter — the simulated device's Trotter error is 1.4e-05 relative at reps=12,
far below the estimator's own, and accuracy is flat across reps 1..24 (`v102`). But
`v103` measured reps=12 costing **3.4x under depolarising noise** at p2=1e-3 while
reps=1 does not, so prefer 1 in any noisy run.

**Under noise the failure mode is the recoverable one** (`v103`): across `p2` from
0 to 1e-2 the cosine to truth moves **0.0045** while the best global scale falls
0.983 -> 0.630. Noise contracts the estimate, it does not rotate it, and at p2=1e-3
the shape error is statistically identical to noiseless. *Not shown*: that the
scale is recoverable without ground truth. *Not tested*: T1/T2 idle decay on the
register during the device evolution — the omission most likely to flatter all of
the above.

### Against the frontier

`v113` measures `twirl_cal` on the axis the literature uses — total evolution time
to precision eps, with `T_total = circuits * shots * T`:

```
fitted   eps ~ T_total^(-0.217)
SQL / arXiv:2606.19486  Theta(Lambda/eps^2)  ->  -0.500
Heisenberg-limited      O(1/eps)             ->  -1.000
```

Worse than SQL, though the global fit is dragged by bias-floored large-T points;
locally, in the shot-limited regime, 4x budget gives 1.76x against SQL's 2.0.

**The row placement matters more than the number.** `twirl_cal` takes `terms` as an
argument and cannot discover an unnamed Pauli, so it solves the *known-structure*
problem — Huang et al.'s row, where the frontier is **Heisenberg-limited O(1/eps)
with no ancilla** — while spending 2N ancillas. It solves an easier problem with
more hardware at no better rate.

Worse, that paper's **Stage 2 already does what this does** — *"one dataset
estimates all candidates together"* by classical parity postprocessing — and does
it ancilla-free.

### Why O(1) circuits and arbitrary precision are mutually exclusive

Batching any such protocol to K settings gives

```
Var  =  Var_settings / K   +   E[Var_shots] / S
```

Shots cannot touch the first term, so **K must also scale as eps^-2**. A fixed
O(1) design therefore buys itself a floor — randomisation variance in their case,
systematic bias and aliasing in `twirl_cal`'s. That is what `v113` and `v102`
measured before the reason was understood, and it means *"O(1) circuits"* and
*"matches the proven-optimal protocol"* cannot both be true. This line chose O(1).

The defensible claim is correspondingly narrow: **a heuristic, fixed-design
estimator with O(1) circuit settings and a precision floor, useful at moderate eps
on per-task-billed hardware.** Not a competitor to protocols with sample-complexity
guarantees, and it cannot become one without giving up the fixed design.

## What the design register actually buys

**`v105` — the O(1) fix, and it was an implementation bug, not physics.** `_walsh`
builds its basis as `obs_pauli[n-1-q] if in XYZ else 'Z'`, so all N single-qubit Z
observables produce **the same circuit** and differ only in which qubit's parity
the decode reads. N circuits were being run where one set of counts already held
all N answers. Grouping by measurement basis gives **8 circuits, flat at N=3..8**,
and it is *more* accurate at matched total shots (0.0814 against 0.1238 at N=4)
because every shot now feeds N observables' marginals instead of one.

**Two bases suffice for any Pauli Hamiltonian.** A term invisible to all-Z lies in
`{I,Z}^N`; invisible to all-X lies in `{I,X}^N`; the intersection is the identity
alone. Coverage verified M/M at N=3..8. What stays family-dependent is the probe
count, since `[P_k,O] != 0` is an operator condition while `<i[P_k,O]> != 0` is a
statement about the probe.

**`v107` — the register carries no information.** An ancilla-free full factorial
reaches 0.0543 against the register's 0.0748 at matched total shots, on **3 qubits
instead of 9**. But it needs *all* 4^N frames, and no fraction works: K=32 random
gives 1.1035, K=32 *distinct* gives 1.0977 (removing duplicates changes nothing),
and all 64 gives 0.0543. The failure is **orthogonality**, not sampling — the
decode is a plain average, unbiased only when sigma is uniform over the whole
group. So the register's real function is **exponential circuit compression**, not
information, which is a stronger statement than `twirl_cal`'s docstring made and a
narrower one than the O(1) claim implied.

**`v117` — shrinking the register costs more than it saves.** Twirling over a
*subgroup* keeps the decode unbiased (characters of a subgroup stay orthogonal,
unlike `v107`'s random fractions), and the qubit saving is real: d = 4,4,5,5,6,6 at
N=3..8 against 2N = 6..16, at or one above the information bound. But it is bought
twice: controlled gates **16 -> 54** at N=8, and alias triples **9 -> 33** at N=4.
*(The file's own first draft asserted aliasing would be unchanged, reasoning that
`v_j+v_k=v_m => Bv_j+Bv_k=Bv_m`. That implication is one-directional; B is
injective only on the M points and it creates new coincidences. Measurement caught
it.)* A qubit saving paid for in the currency that was already scarce.

## Two structural results: symplectic aliasing and the pre-flight

**`v106` — the design is confounded at degree 2, and the columns cannot be
re-chosen.** `sigma_j sigma_k` is itself a Walsh character — the one indexed by
`v_j + v_k` — so when that sum equals some `v_m` in the term set, a degree-2 effect
is indistinguishable from term m's degree-1 effect. For crosstalk
**`v_XX + v_YY = v_ZZ` on every bond** (12 triples at N=3).

`v101`'s full-rank theorem is correct and does not cover this: it proves the M
degree-1 columns independent, which says nothing about a degree-2 product landing
on one of them. Consequences: the measured T^2 bias is **aliasing, not truncation**;
Richardson still removes it (it cancels any T^2 term whatever the origin) but not
for the stated reason; and unlike QLTO's register, where `v90` bought resolution V
with more design rows, **here the columns ARE the Paulis**.

`v106` also corrected `twirl_cal`'s stated reason for using an observable readout.
The docstring said the return probability "has NO first-order term in sigma". Its
degree-1 Walsh coefficient is nonzero and scales as **T^2** (fitted slope 1.986
against the observable's 1.088) — suppressed, not absent. Conclusion unchanged,
reason replaced. *(I predicted slope 3 from an order-T^3 argument. It is 2, via the
aliasing above.)*

And **QPE cannot substitute for the readout**, for a stronger reason than cost: the
twirl acts by conjugation, so `H_sigma` is **isospectral** to H at every sigma —
measured worst spectral deviation 1.4e-15 over all 64 register values, and a phase
readout's degree-1 coefficient is 2.1e-16, machine zero. V5's QPE worked because
its register changed the *state*; this register changes the Hamiltonian by
conjugation, so eigenvectors rotate and eigenvalues do not.

**`v116` — one GF(2) check predicts two obstructions.** Two walls hit several
experiments apart are the same algebraic fact. With `V` the M x 2N matrix of
symplectic vectors, conjugation by `Q(w)` flips term k iff `(Vw)_k = 1`. If any
subset S has `sum_{k in S} v_k = 0` then the flip parity over S is even for *every*
twirl, so:

- **odd |S|** -> all-minus unreachable -> **no time-reversing twirl** (see the
  kernel section below)
- the same null combinations are the **degree-2 aliasing** relations

```
crosstalk N=3..5    alias triples 6/9/12   odd nulls 4/6/8   Q: NO
single-Z, mixed independent, TFIM-like     odd nulls 0       Q: YES
ZZ ring N=3         alias triples 1        odd nulls 1       Q: NO
```

`rank(V) <= 2N`, so `M > 2N` forces dependencies — but **`rank(V) < M` is the real
criterion**, not the M-vs-2N heuristic: the ZZ ring has M=3 < 2N=6 and still fails,
at rank 2. `preflight(terms)` returns all of it in milliseconds. Two builds were
lost this session to questions it answers instantly.

## The kernel import, and why it stalled

`twirl_cal` reads at one fixed T and divides by it, which carries an irreducible
O(T^2) bias. arXiv:2606.19486 removes it with a **band-limited derivative kernel**:
the correlation function has Fourier support in `[-2L, 2L]`, so its derivative at
zero is recoverable *exactly* from finite times. No small-T limit, hence no
truncation bias.

**`v114` builds and verifies the kernel** — 4/4 checks. On the real crosstalk
correlator it recovers `F'(0)` to **9.5e-09**. The discretisation identity holds to
5e-09 below `pi/6L` and **breaks by 78% above it**, which is what makes it evidence
rather than a coincidence of the test function. A `psi` sign error was caught by
Check 2 that Check 1 alone would have shipped — the naive reading gives a kernel
correct in magnitude to 1e-8 that returns **minus** the derivative.

**`v115` failed at the prerequisite.** The kernel is odd, `twirl_cal`'s correlator
has `F(0)=+0.400`, so half-range sampling needs **negative evolution times — time
reversal**, which an always-on device cannot supply. Cross-Pauli readout fixes it
(`IIY`, `YYY`, `IZY` all give `F(0)=0` and exact oddness), which means **the
odd-|A xor B| visibility rule in their Algorithm 2 is not bookkeeping** — it is
what makes the protocol runnable without time reversal.

A twirl `Q` with `QHQ^dag = -H` would supply the negative times for free. `v116`
says when one exists, and for crosstalk it does not — blocked by the same
`v_ZZ + v_XX + v_YY = 0` dependency that causes the aliasing.

Importing the kernel therefore is not a patch: it needs the cross-Pauli
restructure, the cyclic `(c,a,b)` rule and the `q(u)` reweighting — i.e. their
Algorithm 2 with a register in front. And per the `Var_settings/K` argument above,
doing so would take the O(1) claim with it.

## Tunneling on a circuit — and the correction to it

`v99` part 4 is the load-bearing result behind every walk-advantage idea here: a
thin tall barrier drives classical annealing from 1.00 to 0.00 as height goes
2 -> 20 while quantum transmission stays flat at ~5e-3. **It is tier C and
unlabelled** — `expm` on a 21x21 dense matrix, no circuit, and `v99` predates
`CLAUDE.md` by a day. R1 says build it.

**`v118` built it and I drew the wrong conclusion.** Decomposing `diag(E(w))` into
Z-strings shows a thin barrier is many-body: k90 = 7 at width 0.5 against k90 = 2 at
width 8.0, i.e. **254 of a possible 254** Pauli-Z strings at n=8. I concluded the
mechanism is self-defeating — that the thinness enabling tunneling is what makes
the potential exponentially expensive.

**`v118b` refutes that.** A diagonal phase oracle is not built from Pauli
rotations. The standard route is reversible arithmetic: compute w into an ancilla
with an adder, phase-kick, uncompute. Measured, one potential step at n=8:

```
width    Pauli route            arithmetic route
  0.5    255 gates (k<=8)       1045 gates, 897 depth, 8 ancillas
  8.0    162       (k<=4)       1045
```

**Flat.** Identical circuit, different rotation angles. The Pauli route grows
toward 2^n as the barrier thins; the arithmetic route is poly(n) always. At n=8 the
naive route is still cheaper absolutely (255 vs 1045), but it is the one that blows
up, with crossover near n=12.

So `v118` part 1 measures a true fact about the **Pauli decomposition** and a false
one about the **circuit**. Conflating the two was the error.

What survives is weaker: Trotter steps scale as `T * ||H||` with `||H|| ~ h`, so
taller barriers cost more coherent depth. And `v118` part 2 does **not** settle
`v99`'s flatness claim — T was fixed at 20 where `v99` maximised over
{5,10,20,40,80}, n=8 against `v99`'s 20, and the Trotter columns are not converged
(h=20 gives 0.0118 at reps=40 and 0.0196 at reps=160, non-monotone).

**Net: the walk-ML direction is not closed.** Glued trees (Childs et al., STOC
2003) is a proven exponential speedup, and it works because the Hamiltonian is a
*sparse graph adjacency with an efficient oracle* — the speedup and the
implementability come from the same structural fact. The real constraint on a
walk-ML landscape is that it be **both hard for descent and efficiently
computable**, which is a design constraint rather than an obstruction.

## Predictions overturned by measurement

Kept as a list because the pattern is the finding. In each case the argument was
locally valid and omitted the binding constraint.

| predicted | measured |
|---|---|
| random frame sampling would match the register (`v107`) | 15x worse; only the *full factorial* matches — orthogonality, not sampling |
| the register buys mere compilation convenience | it buys exponential circuit compression |
| optimal radius exponent falls with M (`v111`) | it does not; `p=0` collapses to cos 0.095 at M=64 |
| subgroup compression leaves aliasing unchanged (`v117`) | 9 -> 33 triples; the implication was one-directional |
| `psi = +i w chi` (`v114`) | returns minus the derivative; caught by Check 2, not Check 1 |
| return probability degree-1 scales as T^3 (`v106`) | T^2, via degree-2 aliasing |
| half-range kernel sampling would be valid (`v115`) | invalid; needs cross-Pauli readout |
| thin barriers are exponentially expensive to build (`v118`) | flat in width via arithmetic oracle (`v118b`) |

The record is more trustworthy than the reasoning that produced it. That is the
property R1 exists to get, and it is why the tier labels are worth the friction.
