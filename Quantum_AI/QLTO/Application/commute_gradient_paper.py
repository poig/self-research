...
obtained from 2B − 1 circuits (Appendix B, Fig 7):
  ...
  with:
    ...
    W̃  = Gⱼ · W · Gⱼ           (exact; implemented as Pauli sandwich G·W·G)
...
so W̃_j = Gⱼ·W·Gⱼ is QUBIT-SPECIFIC.  We therefore build W̃_j exactly per
generator as a structured Pauli sandwich around W (no dense unitary matrices).
...
class CommutingBlockGradient:
    ...
    NEW  Exact LCU construction (Appendix B, Eq 59–63):
         ...
         |ϕ_b⟩ prepared by H → ctrl-W̃ → ctrl-W' → H circuit (Fig 7)
            W̃ = Gⱼ·W·Gⱼ computed exactly as a Pauli sandwich around W
    """
    ...
    # ── Exact W̃ construction (structured) ─────────────────────────────────

    def _build_W_tilde_gate(self, gen_pauli_type: str, gen_qubit: int,
                            W_gate) -> object:
        """Build W̃ = Gⱼ · W · Gⱼ as a structured gate (no dense matrices)."""
        n = self.ansatz.num_qubits
        qc = QuantumCircuit(n, name='W~')

        if gen_pauli_type == 'X':
            qc.x(gen_qubit)
        elif gen_pauli_type == 'Y':
            qc.y(gen_qubit)
        else:
            qc.z(gen_qubit)
    ...
    def _build_lcu_circuit(self, u_past: QuantumCircuit,
        ...
        # This avoids relying on controlled-unitary global-phase edge cases.
    ...
    def compute_gradient(self, estimator, params_values):
        ...
        for l_idx, layer in enumerate(self.layers):
            ...
            # ── INTERMEDIATE BLOCKS: exact LCU construction (Theorem 3) ─────
                ...
                # Build W once per layer as a structured (decomposable) gate.
                # This avoids dense Operator→UnitaryGate synthesis which is
                # exponential and very slow under MPS simulation.
                try:
                    W_gate = u_future.to_gate(label='W')
                except Exception:
                    # If we can't even convert W to a gate, fall back entirely.
                ...
                for p_local, p_global in enumerate(idxs):
                    ...
                    # ── Build W̃ = Gⱼ · W · Gⱼ exactly (structured) ─────────
                    try:
                        W_tilde_gate = self._build_W_tilde_gate(gen_type, gen_qubit, W_gate)
                    except Exception:
