        ...
        if SKLEARN_AVAILABLE:
            with tqdm(total=100, desc="  🎯 Fast rank (SVD)", ncols=100, leave=True,
                ...
                try:
                    ...
                    # Reduced n_comp from 200 to 50 for a much faster estimate.
                    n_comp = min(50, n_vars - 1)
...
if __name__ == "__main__":
    ...
    # Loop to show resource calculation for k* from 2 to 16
    for k_val in range(2, 33, 4):
        n_val = k_val * 10  # Keep N proportional to k* for demonstration
        print(f"Example: k*={k_val}, N={n_val}")
        print("-"*80)
        resources = recommend_qaoa_resources(k_star=k_val, n_vars=n_val, target_success_rate=0.99)
        print(f"  Partition size: {resources['partition_size']} variables")
        print(f"  Number of partitions: {resources['n_partitions']}")
